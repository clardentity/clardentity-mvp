...
_FORMATTING_RULES = (
    ...
    "prices, dates, features, what each includes - lay that comparison out as a "
    "table, in plain text: a header line, then one line per thing, cells separated "
    "by ' | ' (for example 'Operator | Price | Includes'). Put the whole table "
    "inside ONE <claim> tag, cite inside the cells with [n] as usual, and keep the "
    "prose around it to what the table does not say. Fill every cell you have a "
    "figure for; write 'not stated' where the context has none rather than leaving "
    "the thing out. No tables for anything that is not a comparison.\n"
